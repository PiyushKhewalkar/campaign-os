...
const CalendarDayView = () => null; // Day view disabled
...
const CalendarWeekView = () => {
  ...
  return (
    <div className="flex flex-col relative overflow-auto h-full">
      <div className="grid grid-cols-7 gap-2 sticky top-0 bg-card z-10 border-b p-2 mb-2">
        {headerDays.map((d, i) => (
          <div
            key={d.toString()}
            className={cn('text-sm text-muted-foreground flex items-center gap-1', [0, 6].includes(i) && 'text-muted-foreground/50')}
          >
            {format(d, 'E', { locale })}
      ...
      <div className="grid grid-cols-7 gap-px flex-1">
        {weekDays.map((d, i) => {
          ...
          return (
            <div
              key={d.toString()}
              className={cn('p-2 text-sm text-muted-foreground ring-1 ring-border min-h-28', [0, 6].includes(i) && 'bg-muted/50')}
            >
